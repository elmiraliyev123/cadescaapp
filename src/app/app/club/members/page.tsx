...
export default async function ClubMembersPage({ searchParams }: { searchParams: Promise<{ clubId?: string }> }) {
  try {
    const { clubId } = await searchParams;
    const dashboard = await getCurrentClubDashboard(clubId);
    if (!dashboard || !hasClubCapability(dashboard.roles, "club.members.manage")) return <EventsRouteError error="club_access_denied" />;
    return <ClubMembersView dashboard={dashboard} />;
  } catch (error) {
